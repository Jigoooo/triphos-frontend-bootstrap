...
- JSON-LD is mandated for `__root.tsx` (Organization + WebSite). Root head must not call `buildMeta`; canonical links and page meta are route-owned to avoid duplicate canonical links in Lighthouse. Page routes pick the matching schema.org type via `jsonLdWebPage` (default), `jsonLdArticle` (blog/news), or `jsonLdBreadcrumbs` (deep navigation), wrapped with `jsonLdScript()`.
