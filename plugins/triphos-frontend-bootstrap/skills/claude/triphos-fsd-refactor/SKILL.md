...
<Purpose>
Feature-Sliced Design 원칙에 따라 React/TypeScript 코드베이스를 진단하고 리팩토링하는 스킬. 세그먼트 오분류, 레이어 경계 위반, Public API 우회, 슈퍼 컴포넌트/훅, 200줄 초과 파일, widget 오분류를 잡아낸다.
</Purpose>

<Use_When>
- 사용자가 명시적으로 요청: "FSD로 리팩토링", "이 파일 FSD 원칙에 맞게 정리", "FSD 규칙 위반 찾아줘", `/triphos-fsd-refactor`
- 자동 발동: 프로젝트가 `src/{features,entities,pages,widgets,shared}` 구조이고 사용자가 다음을 요청할 때
  - 파일/폴더 구조 정리
  - 컴포넌트 분리, 훅 추출
  - 리팩토링 (특히 긴 파일을 쪼갤 때)
  - "이 코드 어디에 두는 게 맞아?"
...
<Why_This_Exists>
FSD 위반은 변경 영향 예측 가능성을 깎는다. 레이어 역류, 슈퍼 컴포넌트, essence 기반 폴더는 시간이 지나면서 변경 비용을 폭발시킨다. 이 스킬은 진단 → 계획 → 실행 → 검증 5단계로 그 부채를 통제 가능하게 정리한다.
</Why_This_Exists>
...
<Tool_Usage>
- `references/rules.md`, `references/segments.md`, `references/layer-boundaries.md`, `references/anti-patterns.md` — 진단 결정의 진실 소스
- `scripts/audit-ui-components.mjs` — 자동 UI/hook audit (Phase 3.0 필수)
- `pnpm check` — 타입체크 + lint + 빌드 통합 검증 (커밋 전 필수)
- `git mv` + grep으로 import 참조 추적 후 일괄 업데이트
- `triphos-react-lint-rules` 스킬 — 훅 추출/분리와 동시 적용
</Tool_Usage>

<Escalation_And_Stop_Conditions>
- Phase 3 완료 게이트(audit 실행 + orchestration hook 스캔 + 모든 후보 분류)를 충족하기 전 Phase 4 진행 금지.
- 사용자 승인 없이 리팩토링 코드 변경 금지.
- 규칙이 이상하게 느껴지면 **우선** `references/rules.md`의 "왜" 섹션을 다시 읽는다. 정당한 예외면 사용자에게 명시적으로 설명하고 동의를 받는다. 예외가 반복되면 규칙 자체 재검토를 보고한다. **말없이 어기지 마라.**
- 이동만으로 완료 처리 금지 — 위치만 바꾸고 200줄/구성적 SRP 후보가 남으면 같은 commit에서 분리하거나 `defer` 리포트.
</Escalation_And_Stop_Conditions>

<Final_Checklist>
- [ ] `references/rules.md` 및 필요한 보조 references 모두 읽음
...
- [ ] 사용자 승인된 리팩토링 계획 존재
- [ ] Layer 역류부터 commit 단위로 정리, `pnpm check` 통과
- [ ] 훅 추출/분리에 `triphos-react-lint-rules` 규칙 동시 적용
- [ ] Phase 6 재검증(`audit-ui-components.mjs`, `pnpm check`, 파일 크기, public API)
</Final_Checklist>
...
<Handoff>
- 훅 lint / React Compiler 정리 / useMemo/useCallback 제거 → `triphos-react-lint-rules` (Phase 5 동시 적용)
- 리팩토링 commit 생성 시 → `commit` 스킬
- 리팩토링 후 타입/빌드 실패 → `ts-check` / `build-fix`
- raw API migration이 함께 필요 → `triphos-api-client-setup`
- baseline 자체가 부족 (verifier 없음) → `triphos-frontend-adopt`
</Handoff>
