...
2. `align-runtime`
   ...
   hook 파일과 `.codex/hooks.json` / `.claude/settings.json`은 plugin이 발행하는 `sync-hooks` 스크립트로 byte-equal 동기화한다 — 먼저 dry-run으로 차이를 점검하고, working tree가 깨끗할 때만 `--apply`로 덮어쓴다 (init 시점 snapshot이 stale해지는 P0 위험을 막는다):
   ```
