...
description: fsd-refactor의 rules.md와 보조 reference를 FSD 공식 docs 최신 버전과 대조해 드리프트 리포트 생성. 공식 변경, 사용자 의도적 차이, 신규 권장사항을 분류하고 승인 후에만 업데이트. 수동 전용 (/triphos-fsd-skill-update).
...
`triphos-fsd-refactor` 스킬의 `rules.md`와 보조 reference 파일을 Feature-Sliced Design 공식 문서와 동기화 확인하는 스킬.
...
시간이 지나면 공식 FSD 문서가 업데이트되고, 사용자의 FSD reference와 공식 사이에 드리프트가 생긴다. 이 스킬은 그 드리프트를 **리포트**하여 사용자가 규칙을 의식적으로 최신화하거나 "공식은 바뀌었지만 나는 원래 방식을 유지하겠다"는 결정을 내리도록 돕는다.

**중요**: 이 스킬은 reference 파일을 자동으로 수정하지 않는다. 사용자 승인 후에만 업데이트한다. FSD reference는 사용자의 의식적 결정이 담긴 계약서이지, 공식 문서의 복사본이 아니다.

## 워크플로우

### Phase 1 — 현재 reference 로딩

다음 파일을 처음부터 끝까지 읽는다.

- `../triphos-fsd-refactor/references/rules.md`
- `../triphos-fsd-refactor/references/segments.md`
- `../triphos-fsd-refactor/references/layer-boundaries.md`
- `../triphos-fsd-refactor/references/anti-patterns.md`

각 섹션의 **규칙**, **왜(이유)**, **적용 파일**을 별도로 정리한다. "왜" 섹션은 사용자가 의식적으로 내린 결정이므로, 공식과 달라도 함부로 덮어쓰면 안 된다.
...
Context7이 실패하면 firecrawl MCP로 `https://feature-sliced.github.io/documentation/`을 fallback으로 사용한다. MCP가 없는 런타임에서는 공식 FSD 사이트만 `WebFetch`/`WebSearch` fallback으로 사용하고, 블로그/2차 문서는 근거로 삼지 않는다.

### Phase 3 — 비교

각 주제별로 사용자의 FSD reference 파일들과 공식을 대조하여 다음 4개 카테고리로 분류한다. 각 항목에는 대상 파일(`rules.md`, `segments.md`, `layer-boundaries.md`, `anti-patterns.md`)을 반드시 적는다.
...
### Phase 5 — 승인 후 reference 업데이트

사용자가 적용할 항목을 선택하면:

1. 해당 섹션만 대상 reference 파일에서 edit.
2. (B) 카테고리의 결정은 건드리지 마라. "왜" 섹션에 이유가 있는 규칙은 사용자가 명시적으로 "공식 따라가겠다"라고 말할 때만 변경.
3. 변경된 섹션마다 **간단한 주석**으로 "[공식 docs 기준 YYYY-MM-DD 동기화]" 같은 footnote를 달지 마라 — reference 파일은 깔끔하게 유지한다.
4. 업데이트 후 사용자에게 diff를 보여주고 최종 확인.

## 누락 방지 게이트

- Phase 4 리포트에 네 reference 파일이 모두 포함됐는지 확인한다.
- 공식 docs 수집 실패, 일부 주제 누락, MCP/tool fallback 사용 여부를 리포트에 남긴다.
- 승인 없이 파일을 수정하지 않는다. 승인 후에도 선택된 항목과 대상 파일 외에는 수정하지 않는다.
- 수정 후 `pnpm run check:parity`를 실행해 Claude/Codex reference drift가 없는지 확인한다.
...
- MCP가 모두 실패하면 공식 FSD URL만 WebFetch/WebSearch로 fallback한다. 2차 문서는 사용하지 않는다.
...
- `../triphos-fsd-refactor/references/rules.md` — 주요 규칙
- `../triphos-fsd-refactor/references/segments.md` — 세그먼트 분류
- `../triphos-fsd-refactor/references/layer-boundaries.md` — import 방향과 layer 경계
- `../triphos-fsd-refactor/references/anti-patterns.md` — 리팩토링 anti-pattern
